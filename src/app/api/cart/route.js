...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { success: false, message: error.message || "Unable to load cart." },
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: error.message || "Unable to add item to cart." },
      { status: error.statusCode || 500 }
    );
  }
}
...
export async function PUT(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: error.message || "Unable to update cart item." },
      { status: error.statusCode || 500 }
    );
  }
}
...
export async function DELETE(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: error.message || "Unable to remove cart item." },
      { status: error.statusCode || 500 }
    );
  }
}
